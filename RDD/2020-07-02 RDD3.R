...
# Read in the data
data <-read_dta("data_meyersson.dta") #haven
...
plot1 = ggplot(data=data, aes(X)) + theme_bw(base_size = 14) +
  geom_histogram(data = data, aes(x = X, y= ..count..), breaks = seq(-bw, 0, 1), fill = "blue", col = "black", alpha = 1) +
  geom_histogram(data = data, aes(x = X, y= ..count..), breaks = seq(0, bw, 1), fill = "red", col = "black", alpha = 1) +
...
# Col (1) above median 
out = rdrobust(Y[vshr_islam1994 >= median], X[vshr_islam1994 >= median], covs = covariates, kernel = "triangular", p = 1, h = 25)
summary(out)
