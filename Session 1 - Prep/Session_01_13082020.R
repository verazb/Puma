...
ggplot(data, aes(x = age)) +
        geom_histogram(aes(y = stat(count)), binwidth = 5, col="red", fill="blue", alpha = .2)
ggplot(data, aes(x = insured_earn)) +
        geom_histogram(aes(y = stat(count)), col="red", fill="blue", alpha = .2)
...
# Drop those above 60 (CS!) - Extract rows that meet logical criteria. 
...
save(data_desc, file="data_desc.RData")
